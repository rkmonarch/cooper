...
type Step = "policy" | "confirm" | "paying" | "done" | "error";
...
export function UnlockModal({ listing, open, onClose, onSuccess }: UnlockModalProps) {
  ...
  // Auto-fire for one-tap policy flow
  useEffect(() => {
    if (open && step === "policy" && mounted) handlePay();
  ...
  async function handlePay() {
    ...
    if (!recipient || recipient === "anonymous") {
      setError("This listing has no creator address.");
      setStep("error");
      return;
    }
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 99999, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-[2rem] border border-[var(--border)] bg-[var(--card)] shadow-[0_40px_80px_rgba(54,72,42,0.22)]">
        ...
        <div className="space-y-4 p-6">
          ...
          {step === "policy" && (
            <div className="flex items-center gap-2 rounded-[1rem] border border-lime-200 bg-lime-50/80 px-4 py-3 text-xs text-[var(--success)]">
              ...
              <span>Within your spending policy — paying via OWS agent wallet…</span>
            </div>
          )}
          ...
          {step === "confirm" && (
            <>
              ...
              <div className="flex items-start gap-2 text-xs text-[var(--muted)]">
                ...
                <span>
                  Above your auto-approve threshold. Your OWS agent wallet will sign this transaction server-side.
                </span>
              </div>
            </>
          )}
          ...
          {step === "paying" && (
            <div className="flex flex-col items-center gap-3 py-4">
              ...
              <p className="text-sm font-semibold text-[var(--foreground)]">OWS agent wallet signing…</p>
              <p className="text-xs text-[var(--muted)]">Broadcasting USDC transfer to Solana devnet</p>
            </div>
          )}

          {/* Error */}
          {step === "error" && error && (
            <div className="rounded-[1rem] border border-red-100 bg-red-50 p-4 space-y-2">
              <p className="text-sm font-semibold text-red-600">{error}</p>
              {(error.toLowerCase().includes("usdc") || error.toLowerCase().includes("faucet")) && (
                <a href="https://faucet.circle.com" target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-1 text-xs font-bold text-red-500 underline underline-offset-2">
                  Get devnet USDC at faucet.circle.com →
                </a>
              )}
              {error.toLowerCase().includes("sol") && (
        ...
        <div className="flex gap-3 px-6 pb-6">
          {step === "done" && (
            <Button className="flex-1" onClick={() => onSuccess(content ?? "")}>Done</Button>
          )}
          {step === "confirm" && (
            <>
